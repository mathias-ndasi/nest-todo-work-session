...
export class TodoServise {
  ...
  async findTodos(params: GetTodoParams): Promise<ResponseWithData> {
    try {
      const validateGetTodos = await this.todoValidator.validateGeTodosParams(
        params,
      );
      if (validateGetTodos.status !== HttpStatus.OK) return validateGetTodos;

      const limit =
        params.limit && Number(params.limit) <= Constants.LIMIT
          ? Number(params.limit)
          : Constants.LIMIT;
      const offset = params.offset ? Number(params.offset) : Constants.OFFSET;

      let filterParams: any = { limit, offset };
      filterParams = params.name
        ? { ...filterParams, email: params.name }
        : filterParams;
      filterParams = params.done
        ? { ...filterParams, username: params.done }
        : filterParams;
  ...
  async findTodo(todoId: number): Promise<ResponseWithData> {
    try {
      const validateGetTodo = await this.todoValidator.validateGetTodoParams(
        todoId,
      );
  ...
  async updateTodo(
    ...
    try {
      const validateTodoUpdate = await this.todoValidator.validateUpdateTodoDto(
        todoId,
        dto,
      );
      if (validateTodoUpdate.status !== HttpStatus.OK)
        return validateTodoUpdate;
  ...
  async deleteTodo(todoId: number): Promise<ResponseWithoutData> {
    try {
      const deleteTodo = await this.todoValidator.validateDeleteTodoParams(
        todoId,
      );
